...
public abstract class ModuleIO implements Logged {
  ...
  protected TrapezoidProfile.Constraints m_steeringConstraints =
      new TrapezoidProfile.Constraints(STEER_MAX_SPEED_RAD_PER_SEC, STEER_MAX_ACCEL_RAD_PER_SEC_SQ);
  ...
  private void setState() {
    ...
    m_steerGoal =
        new State(
            m_steerGoal.position,
            new Rotation2d(m_steerGoal.position).minus(new Rotation2d(prevSteerGoal)).getRadians()
                / 0.02);
    ...
    setRotationPid(
        m_steerSetpoint.position,
        m_steerFeedForward.calculate(
            m_steerSetpoint.velocity)); // m_steerFeedForward.calculate(m_steerSetpoint.velocity,
    // m_nextSetpoint.velocity, 0.02));
  }
